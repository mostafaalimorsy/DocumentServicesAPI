    ...
    public class ExternalDocumentService : IExternalDocumentService
    {
        ...
        public async Task<ExternalFileAsPdfResponse> DownloadFileFromExternalApiAsync(ExternalFileDownloadRequest request, string userToken)
        {
            ...
            if (!urlVersionDataResponse.IsSuccessStatusCode)
            {
                ...
                {
                    Error = "Failed to fetch versions from external service.",
                    Details = $"Status: {urlVersionDataResponse.StatusCode} Response: {urlVersionContent}"
                };
            ...
            // Deserialize
            //var versions = JsonSerializer.Deserialize<List<VersionDto>>(jsonResponse);

            // Sort numerically by parsing the VersionNumber
            var sortedVersions = versions
                .OrderBy(v => double.Parse(v.VersionNumber, CultureInfo.InvariantCulture))
                .ToList();


            if(sortedVersions.Count == 0)
            {
                // If no versions are found, throw an error
                ErrorResponse errorContent = new ErrorResponse
                {
                    Error = "No versions found for the specified document.",
                    Details = "The document does not have any versions available."
                };
                throw new ErrorException(errorContent);
            }
            // Get the highest version
            var latestVersion = sortedVersions.Last().VersionNumber;
            ...
            if (!dataResponse.IsSuccessStatusCode)
            {
                ...
                ErrorResponse errorContent = new ErrorResponse
                {
                    Error = "Failed to fetch file data from external service.",
                    Details = $"Status: {dataResponse.StatusCode} Response: {content}"
                };
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                ErrorResponse errorResponse = new ErrorResponse
                {
                    ...
                    Details = $"Status: {response.StatusCode} Response: {error}"
                };
